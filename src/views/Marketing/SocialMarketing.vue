  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Social Marketing" />
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(API_BASE_URL + '/marketing/social_marketing', {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? API_BASE_URL + '/marketing/social_marketing/' + formData.value.id : API_BASE_URL + '/marketing/social_marketing'
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(API_BASE_URL + '/marketing/social_marketing/' + id, {
      method: 'DELETE',
