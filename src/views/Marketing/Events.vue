          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Acara</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Lokasi</p></th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ (record as any).event_name || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ (record as any).location || '-' }}</p></td>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-lg dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord">
        ...
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Acara</label>
          <input v-model="formData.event_name" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Lokasi</label>
          <input v-model="formData.location" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          ...
          <select v-model="formData.status" class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            <option value="Planning">Planning</option><option value="Ongoing">Ongoing</option><option value="Finished">Finished</option>
          </select>
        </div>
...
import type { IEventDto } from '@/types/marketing'
...
const formData = ref({ id: null, event_name: '', location: '', status: 'Planning' })
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, event_name: data.event_name || '', location: data.location || '', status: data.status || 'Planning' }
  } else {
    formData.value = { id: null, event_name: '', location: '', status: 'Planning' }
  }
