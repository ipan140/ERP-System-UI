<template>
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10 space-y-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
          ...
          <h3 class="mt-2 text-xl font-bold text-amber-600 dark:text-amber-400">{{ formatRupiah(totalUtmRevenue) }}</h3>
        ...
        <div class="max-w-full overflow-x-auto custom-scrollbar">
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="isLoading"><td colspan="5" class="px-5 py-8 text-center text-gray-500">Memuat data kampanye...</td></tr>
              <tr v-else-if="error"><td colspan="5" class="px-5 py-8 text-center text-red-500">{{ error }}</td></tr>
              <tr v-else-if="records.length === 0"><td colspan="5" class="px-5 py-8 text-center text-gray-500">Belum ada kampanye email yang dibuat.</td></tr>
              <tr v-for="record in records" :key="record.id" class="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 transition">
                <td class="px-5 py-4">
                  <div class="font-semibold text-gray-900 dark:text-white">{{ record.name || record.subject || 'Tanpa Judul' }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 flex items-center gap-1">
                    <span>Subjek:</span> {{ record.subject || '-' }}
                  </div>
                </td>
                <td class="px-5 py-4">
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto">
    <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord" class="mt-4 space-y-4">
        ...
        <div>
          <label class="mb-1 block text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-300">Subjek Email (Muncul di Inbox Penerima)</label>
  ...
  <div v-if="isUtmModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto">
