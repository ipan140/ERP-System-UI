  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Marketing Automation" />
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(API_BASE_URL + '/marketing/marketing_automation', {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? API_BASE_URL + '/marketing/marketing_automation/' + formData.value.id : API_BASE_URL + '/marketing/marketing_automation'
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(API_BASE_URL + '/marketing/marketing_automation/' + id, {
      method: 'DELETE',
