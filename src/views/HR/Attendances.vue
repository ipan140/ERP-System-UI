...
const formData = ref({ id: null, employee_id: null, date: '', status: 'Hadir' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/attendances`, {
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, employee_id: data.employee_id || null, date: data.date || '', status: data.status || 'Hadir' }
  } else {
    formData.value = { id: null, employee_id: null, date: '', status: 'Hadir' }
  }
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL}/hr/attendances/${formData.value.id}` : `${API_BASE_URL}/hr/attendances`
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/attendances/${id}`, {
      method: 'DELETE',
