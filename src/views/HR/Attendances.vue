...
import type { IAttendanceDto } from '@/types/hr'
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await attendancesService.update(formData.value.id, formData.value)
    } else {
      await attendancesService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
