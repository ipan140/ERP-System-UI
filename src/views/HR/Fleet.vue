    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 mb-6">
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm relative overflow-hidden">
          ...
          <h4 class="text-2xl font-bold text-gray-900 dark:text-white relative z-10">{{ allRecords.length }} Unit</h4>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm relative overflow-hidden">
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1 relative z-10">Kendaraan Aktif</p>
          <h4 class="text-2xl font-bold text-green-500 relative z-10">{{ allRecords.filter(r => r.state === 'Active').length }} Unit</h4>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm">
          ...
          <h4 class="text-2xl font-bold text-orange-500">{{ allRecords.filter(r => r.state === 'Maintenance').length }} Unit</h4>
        </div>
      </div>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            ...
            <div class="relative flex-1 sm:flex-initial">
              <input v-model="searchQuery" type="text" placeholder="Cari model / plat..." class="w-full sm:w-64 rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 pl-10 text-sm focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white/90" />
...
const allRecords = ref<IVehicleDto[]>([])
...
const fetchAllMetrics = async () => {
  try {
    const res = await fleetService.getAll({ all: 'true' })
    allRecords.value = Array.isArray(res) ? res : (res?.data || [])
  } catch (e) { console.error('Failed to load fleet metrics', e) }
}
...
const fetchPaginatedData = async () => {
  ...
  try {
    ...
    const res = await fleetService.getAll(params)
    if (res && res.data) {
