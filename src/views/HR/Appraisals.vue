  ...
  <AdminLayout>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      ...
      <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <PageBreadcrumb pageTitle="Penilaian Kinerja (Appraisals)" />
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 mb-6">
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm">
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Rata-rata Skor Keseluruhan</p>
          <div class="flex items-center gap-2">
            <h4 class="text-2xl font-bold text-gray-900 dark:text-white">{{ averageScore.toFixed(1) }}</h4>
            <span class="text-yellow-400 text-xl">★</span>
          </div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm">
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Total Penilaian Selesai</p>
          <h4 class="text-2xl font-bold text-gray-900 dark:text-white">{{ completedAppraisals }}</h4>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white dark:bg-white/[0.03] p-5 dark:border-gray-800 shadow-sm">
          <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Menunggu Review</p>
          <h4 class="text-2xl font-bold text-orange-500">{{ pendingAppraisals }}</h4>
        </div>
      </div>
          ...
          <table class="min-w-full">
            <thead>
              <tr class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                <th class="px-5 py-3 text-left"><p class="font-semibold text-gray-600 text-xs uppercase dark:text-gray-400">Pegawai & Manajer</p></th>
                <th class="px-5 py-3 text-left"><p class="font-semibold text-gray-600 text-xs uppercase dark:text-gray-400">Tanggal</p></th>
                <th class="px-5 py-3 text-left"><p class="font-semibold text-gray-600 text-xs uppercase dark:text-gray-400">Skor (1-5)</p></th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              <tr v-else-if="records.length === 0">
                <td colspan="5" class="py-10 text-center text-gray-500">Belum ada data penilaian.</td>
              </tr>
              <tr v-for="record in records" :key="record.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800/20">
                <td class="px-5 py-4">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center font-bold text-sm shrink-0">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-gray-500 dark:text-gray-400">Dinilai oleh: {{ record.manager?.name || `Manajer #${record.manager_id || '-'}` }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-4">
                  <span class="text-sm text-gray-700 dark:text-gray-300">{{ formatDate(record.date || record.created_at) }}</span>
                </td>
                <td class="px-5 py-4">
                  <div class="flex items-center gap-1 text-yellow-400" title="Skor Penilaian">
                    <svg v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= (record.score || 0) ? 'fill-current' : 'text-gray-300 dark:text-gray-600 fill-current'" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                    <span class="ml-1 text-xs font-medium text-gray-500 dark:text-gray-400">({{ record.score || 0 }}/5)</span>
                  </div>
                </td>
                <td class="px-5 py-4">
      ...
      <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-gray-800 my-8 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
          <h3 class="text-lg font-bold text-gray-900 dark:text-white">
            {{ modalMode === 'create' ? 'Tambah Penilaian Baru' : 'Edit Penilaian' }}
          </h3>
        ...
        <form @submit.prevent="saveRecord">
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">ID Manajer</label>
                <select v-model.number="formData.manager_id" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                  <option :value="undefined">-- Pilih Manajer (Atasan) --</option>
                  <option v-for="emp in employeesList" :key="emp.id" :value="emp.id">{{ emp.name }}</option>
                </select>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Penilaian</label>
              ...
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Skor (1-5)</label>
                <input v-model.number="formData.score" type="number" min="1" max="5" required class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
              </div>
            </div>
            ...
            <div>
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Feedback / Catatan</label>
              <textarea v-model="formData.feedback" rows="3" class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
            </div>
          </div>
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    ...
    formData.value = { id: undefined, employee_id: undefined, manager_id: undefined, date: formatForInput(new Date().toISOString()), score: 3, feedback: '', state: 'Draft' }
  }
...
const saveRecord = async () => {
  ...
  try {
    ...
    if (modalMode.value === 'edit' && formData.value.id) {
      await appraisalsService.update(formData.value.id, payload)
    } else {
      await appraisalsService.create(payload)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
...
const deleteRecord = async (id: number) => {
  ...
  try {
    await appraisalsService.delete(id)
    fetchData()
  } catch (err: any) {
    alert(err.response?.data?.message || err.message)
  }
