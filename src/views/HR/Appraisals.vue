          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">ID Karyawan</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nilai (1-5)</p></th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.employee_id || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.score || '-' }}</p></td>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-lg dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord">
        ...
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nilai (1-5)</label>
          <input v-model="formData.score" type="number" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Keterangan/Feedback</label>
          <input v-model="formData.feedback" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
...
const formData = ref({ id: null, employee_id: null, score: 5, feedback: '' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/appraisals`, {
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, employee_id: data.employee_id || null, score: data.score || 5, feedback: data.feedback || '' }
  } else {
    formData.value = { id: null, employee_id: null, score: 5, feedback: '' }
  }
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL}/hr/appraisals/${formData.value.id}` : `${API_BASE_URL}/hr/appraisals`
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/appraisals/${id}`, {
      method: 'DELETE',
