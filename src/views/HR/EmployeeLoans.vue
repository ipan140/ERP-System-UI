  ...
  <AdminLayout>
    <div class="p-6">
      <div class="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <PageBreadcrumb pageTitle="Pinjaman Karyawan (Kasbon)" />
        </div>
          ...
          <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
            <thead class="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-gray-700/50 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th class="px-6 py-4 font-semibold text-gray-900 dark:text-white">Tanggal Pengajuan</th>
                <th class="px-6 py-4 font-semibold text-gray-900 dark:text-white">Pokok Pinjaman</th>
                <th class="px-6 py-4 font-semibold text-gray-900 dark:text-white">Tenor (Bulan)</th>
                <th class="px-6 py-4 font-semibold text-gray-900 dark:text-white">Cicilan per Bulan</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="row in records" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td class="px-6 py-4">{{ formatCurrency(row.principal_amount || 0) }}</td>
                <td class="px-6 py-4">{{ row.tenor_months }}</td>
                <td class="px-6 py-4">{{ formatCurrency(row.monthly_installment || 0) }}</td>
                <td class="px-6 py-4">
...
const fetchData = async (page = pagination.value.page, perPage = pagination.value.per_page) => {
  ...
  try {
    const [res, empRes] = await Promise.all([
      http.get('/hr/employees/employeeloan', { params: { page, per_page: perPage } }),
...
const saveRecord = async () => {
  ...
  try {
    ...
    if (payload.principal_amount) payload.principal_amount = Number(payload.principal_amount)
    if (payload.tenor_months) payload.tenor_months = Number(payload.tenor_months)
    if (payload.monthly_installment) payload.monthly_installment = Number(payload.monthly_installment)
    if (payload.date && !payload.date.includes('T')) payload.date += 'T00:00:00Z'

    if (modalMode.value === 'edit' && payload.id) {
      await http.put(`/hr/employees/employeeloan/${payload.id}`, payload)
    } else {
      await http.post('/hr/employees/employeeloan', payload)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
...
const deleteRecord = async (id: number) => {
  ...
  try {
    await http.delete(`/hr/employees/employeeloan/${id}`)
    fetchData()
  } catch (err: any) {
    alert(err.message)
  }
