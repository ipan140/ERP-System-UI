          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Pelamar</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Posisi Dilamar</p></th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.applicant_name || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.position || '-' }}</p></td>
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/hr/recruitment`, {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL}/hr/recruitment/${formData.value.id}` : `${API_BASE_URL}/hr/recruitment`
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/recruitment/${id}`, {
      method: 'DELETE',
