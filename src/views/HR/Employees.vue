          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Lengkap</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Jabatan</p></th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.name || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.job_title || '-' }}</p></td>
    ...
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-lg dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord">
        ...
        <div class="mb-4">
          ...
          <input v-model="formData.job_title" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          ...
          <select v-model="formData.status" class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            <option value="Aktif">Aktif</option><option value="Cuti">Cuti</option><option value="Resign">Resign</option>
          </select>
        </div>
...
const formData = ref({ id: null, name: '', job_title: '', status: 'Aktif' })
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, name: data.name || '', job_title: data.job_title || '', status: data.status || 'Aktif' }
  } else {
    formData.value = { id: null, name: '', job_title: '', status: 'Aktif' }
  }
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL}/hr/employees/${formData.value.id}` : `${API_BASE_URL}/hr/employees`
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/hr/employees/${id}`, {
      method: 'DELETE',
