<template>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="row in filteredRecords" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td class="px-6 py-4 text-right">
                  <button @click="openPayslip(row)" class="text-brand-500 hover:text-brand-700 mr-3">Lihat Rincian</button>
                  <button v-if="row.status === 'draft'" @click="payPayslip(row.id)" class="text-green-500 hover:text-green-700 mr-3">Bayar</button>
  ...
  <Teleport to="body">
    <div v-if="activePayslip" class="fixed inset-0 z-[99999] flex items-center justify-center bg-gray-900/60 p-4 backdrop-blur-sm">
      <div class="w-full max-w-2xl rounded-2xl bg-white shadow-2xl overflow-hidden text-gray-800">
        <!-- Print Area -->
        <div id="payslip-print-area" class="p-8 bg-white">
          <div class="flex justify-between items-start border-b-2 border-gray-200 pb-6 mb-6">
            <div>
              <h1 class="text-3xl font-black text-brand-600 tracking-tight">SLIP GAJI</h1>
              <p class="text-sm font-medium text-gray-500 mt-1">Periode: {{ activePayslip.period }}</p>
            </div>
            <div class="text-right">
              <h3 class="font-bold text-lg text-gray-900">{{ activePayslip.employee?.name }}</h3>
              <p class="text-sm text-gray-600">{{ activePayslip.employee?.job_position?.name }}</p>
              <p class="text-sm text-gray-600">{{ activePayslip.employee?.department?.name }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-8">
            <!-- Earnings -->
            <div>
              <h4 class="font-bold text-green-600 mb-3 border-b border-gray-100 pb-2">PENDAPATAN</h4>
              <div class="space-y-2">
              ...
              <div class="mt-4 pt-3 border-t border-gray-200 flex justify-between font-bold text-sm">
                <span>Total Pendapatan</span>
                <span class="text-green-600">{{ formatCurrency(activePayslip.total_earning) }}</span>
              </div>
            </div>
            
            <!-- Deductions -->
            <div>
              <h4 class="font-bold text-red-500 mb-3 border-b border-gray-100 pb-2">POTONGAN</h4>
              <div class="space-y-2">
              ...
              <div class="mt-4 pt-3 border-t border-gray-200 flex justify-between font-bold text-sm">
                ...
                <span class="text-red-500">{{ formatCurrency(activePayslip.total_deduction) }}</span>
              </div>
            </div>
          </div>
          
          <div class="mt-8 bg-gray-50 rounded-xl p-5 border border-gray-200 flex justify-between items-center">
            <div>
              <p class="text-sm font-medium text-gray-500">Total Dibayarkan (Take Home Pay)</p>
              <h2 class="text-2xl font-black text-brand-600 mt-1">{{ formatCurrency(activePayslip.net_salary) }}</h2>
            </div>
            <div class="text-right">
              <p class="text-xs text-gray-400">Status Pembayaran</p>
              <div :class="['mt-1 inline-flex px-3 py-1 text-sm font-bold rounded-full uppercase tracking-widest', activePayslip.status === 'paid' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200']">
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
          <button @click="activePayslip = null" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100">Tutup</button>
          <button @click="printPayslip" class="px-5 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600">Print / PDF</button>
        </div>
      </div>
    </div>
  </Teleport>
