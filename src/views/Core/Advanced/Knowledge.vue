...
import type { IArticleDto } from '@/types/core'

const records = ref<IArticleDto[]>([])
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await knowledgeService.update(formData.value.id, formData.value)
    } else {
      await knowledgeService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
