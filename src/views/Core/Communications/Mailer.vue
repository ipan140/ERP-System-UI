  ...
  <AdminLayout>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
      <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <PageBreadcrumb pageTitle="Mailer" />
...
import { mailerService } from '@/services/core/mailer.service'
import type { IEmailLogDto } from '@/types/core'

const records = ref<IEmailLogDto[]>([])
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await mailerService.update(formData.value.id, formData.value)
    } else {
      await mailerService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
