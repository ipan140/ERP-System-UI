          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Barang Disewa</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Penyewa</p></th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ (record as any).item_name || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ (record as any).customer_name || '-' }}</p></td>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-lg dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord">
        ...
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Barang Disewa</label>
          <input v-model="formData.item_name" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Penyewa</label>
          <input v-model="formData.customer_name" type="text" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
          ...
          <select v-model="formData.status" class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            <option value="Reserved">Reserved</option><option value="Picked Up">Picked Up</option><option value="Returned">Returned</option>
          </select>
        </div>
...
import type { IRentalOrderDto } from '@/types/sales'

const records = ref<IRentalOrderDto[]>([])
...
const formData = ref({ id: null, item_name: '', customer_name: '', status: 'Reserved' })
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, item_name: data.item_name || '', customer_name: data.customer_name || '', status: data.status || 'Reserved' }
  } else {
    formData.value = { id: null, item_name: '', customer_name: '', status: 'Reserved' }
  }
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await rentalService.update(formData.value.id, formData.value)
    } else {
      await rentalService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
