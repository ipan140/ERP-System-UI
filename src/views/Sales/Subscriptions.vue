          ...
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-gray-800">
                ...
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.customer_name || '-' }}</p></td><td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ record.plan || '-' }}</p></td>
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? `${API_BASE_URL}/sales/subscriptions/${formData.value.id}` : `${API_BASE_URL}/sales/subscriptions`
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`${API_BASE_URL}/sales/subscriptions/${id}`, {
      method: 'DELETE',
