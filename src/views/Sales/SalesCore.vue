  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Sales Core" />
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(API_BASE_URL + '/sales/sales_core', {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? API_BASE_URL + '/sales/sales_core/' + formData.value.id : API_BASE_URL + '/sales/sales_core'
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(API_BASE_URL + '/sales/sales_core/' + id, {
      method: 'DELETE',
