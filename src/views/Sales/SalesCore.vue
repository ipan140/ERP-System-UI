          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                ...
                <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Pelanggan</p></th><th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Total Penawaran</p></th>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-lg dark:bg-gray-800 my-8">
      ...
      <form @submit.prevent="saveRecord">
        ...
        <div class="mb-4">
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Total Penawaran</label>
          <input v-model="formData.total_amount" type="number" required class="w-full rounded-md border border-gray-300 px-3 py-2 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
        </div>
        <div class="mb-4">
...
const formData = ref({ id: null, customer_name: '', total_amount: 0, status: 'Draft' })
...
const openModal = (mode: 'create' | 'edit', data: any = null) => {
  ...
  if (mode === 'edit' && data) {
    formData.value = { id: data.id, customer_name: data.customer_name || '', total_amount: data.total_amount || 0, status: data.status || 'Draft' }
  } else {
    formData.value = { id: null, customer_name: '', total_amount: 0, status: 'Draft' }
  }
