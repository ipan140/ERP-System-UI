    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xs">
      <div>
        <h2 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <span class="p-1.5 rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400 text-sm">📈</span>
          Kalender Penjualan & CRM (Sales & Deals Calendar)
        </h2>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
          Jadwal demo presentasi produk ke klien B2B, agenda follow-up quotation, penandatanganan kontrak, dan evaluasi kuota sales.
        </p>
      </div>
      ...
      <Modal v-if="isOpen" @close="closeModal">
        <template #body>
          ...
          >
            ...
            >
              {{ selectedEvent ? 'Edit Aktivitas Sales' : 'Tambah Aktivitas Sales' }}
            </h5>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              Jadwalkan presentasi penawaran, panggilan negosiasi deal CRM, atau penandatanganan kerja sama klien.
            </p>

            <div class="mt-8">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  Judul Pertemuan / Aktivitas
                </label>
                <input
                  v-model="eventTitle"
                  type="text"
                  placeholder="Contoh: Demo Produk ERP ke PT Sinar Mas / Negosiasi Kontrak"
                  class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                />
              </div>

                <div class="mt-4">
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Catatan Deal / Kebutuhan Klien</label>
                  <textarea v-model="eventDescription" placeholder="Catatan proposal nilai penawaran, kebutuhan fitur prospek, atau nomor quotation" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" rows="3"></textarea>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Lokasi / Kantor Klien</label>
                    <input v-model="eventLocation" placeholder="Menara Astra Lt. 25 / Online" type="text" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Tautan Rapat (Zoom / GMeet)</label>
                    <input v-model="eventMeetingUrl" placeholder="https://meet.google.com/sales-demo" type="url" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <div class="flex items-center justify-between mb-1.5">
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">Kategori Penjualan</label>
                      <button
                        type="button"
                        @click="isCategoryModalOpen = true"
                  ...
                  <div>
                    ...
                    <select v-model="eventVisibility" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                      <option value="private">Privat (Hanya Akun Sales)</option>
                      <option value="public">Publik (Seluruh Tim Sales & Manajer)</option>
                    </select>
                  </div>
                </div>
...
const currentPageTitle = ref('Kalender Penjualan')
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        events.value = [
          {
            id: 'sls-1',
            title: '🤝 Demo Presentasi ERP: PT Astra Graphia Tbk',
            start: `${y}-${m}-11T10:00:00`,
            end: `${y}-${m}-11T12:00:00`,
            allDay: false,
            extendedProps: {
              calendar: 'primary',
              event_type: 'Demo Presentasi B2B',
              description: 'Presentasi kapabilitas modul Supply Chain & Manufacturing untuk 5 entitas anak perusahaan',
              location: 'GMeet / Boardroom Lt. 8',
              meeting_url: 'https://meet.google.com/sales-demo-ag',
              visibility: 'public'
            }
          },
          {
            id: 'sls-2',
            title: '📞 Follow-up Proposal & Negosiasi Harga: CV Sinar Harapan',
            start: `${y}-${m}-17T14:00:00`,
            end: `${y}-${m}-17T15:00:00`,
            allDay: false,
            extendedProps: {
              calendar: 'warning',
              event_type: 'Follow-up Quotation',
              description: 'Follow-up penawaran sistem POS & Akuntansi senilai Rp 150.000.000 (diskon lisensi 10%)',
              location: 'Panggilan Telepon / Zoom',
              visibility: 'private'
            }
          },
          {
            id: 'sls-3',
            title: '✍️ Closing Deal & Signing MOU: Bank Mandiri Enterprise',
            start: `${y}-${m}-21T11:00:00`,
            end: `${y}-${m}-21T13:00:00`,
            allDay: false,
            extendedProps: {
              calendar: 'success',
              event_type: 'Signing Kontrak MOU',
              description: 'Seremoni penandatanganan kontrak implementasi core system perbankan',
              location: 'Plaza Mandiri Lt. 15',
              visibility: 'public'
            }
          },
          {
            id: 'sls-4',
            title: '🎯 Sales Monthly Quota Review & Commission Meeting',
            start: `${y}-${m}-30T15:30:00`,
            end: `${y}-${m}-30T17:30:00`,
            allDay: false,
            extendedProps: {
              calendar: 'danger',
              event_type: 'Review Target Kuota',
              description: 'Evaluasi pencapaian target penjualan bulanan dan perhitungan bonus komisi tim sales',
              location: 'Ruang Rapat Sales Utama',
              visibility: 'public'
            }
          }
        ];
