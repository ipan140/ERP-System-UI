...
import { approvalsService } from '@/services/finance/approvals.service'
import type { IApprovalRequestDto } from '@/types/finance'

const records = ref<IApprovalRequestDto[]>([])
...
const formData = ref<IApprovalRequestDto | any>({ name: '', description: '' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const data = await approvalsService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await approvalsService.update(formData.value.id, formData.value)
    } else {
      await approvalsService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
