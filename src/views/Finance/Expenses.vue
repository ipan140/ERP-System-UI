...
import { expensesService } from '@/services/finance/expenses.service'
import type { IExpenseDto } from '@/types/finance'

const records = ref<IExpenseDto[]>([])
...
const formData = ref<IExpenseDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await expensesService.update(formData.value.id, formData.value)
    } else {
      await expensesService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
