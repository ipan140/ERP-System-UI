    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xs">
      <div>
        <h2 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <span class="p-1.5 rounded-lg bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 text-sm">💰</span>
          Kalender Keuangan, Pajak & Perbankan (Finance & Accounting Calendar)
        </h2>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
          Jadwal jatuh tempo faktur piutang (AR), pembayaran tagihan vendor (AP), batas pelaporan SPT Pajak (PPh/PPN), dan closing buku kas.
        </p>
      </div>
      ...
      <Modal v-if="isOpen" @close="closeModal">
        <template #body>
          ...
          >
            ...
            >
              {{ selectedEvent ? 'Edit Agenda Keuangan' : 'Tambah Agenda Keuangan' }}
            </h5>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              Jadwalkan batas pelaporan pajak, jatuh tempo piutang/hutang, atau jadwal tutup buku akuntansi.
            </p>

            <div class="mt-8">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  Judul Agenda Keuangan
                </label>
                <input
                  v-model="eventTitle"
                  type="text"
                  placeholder="Contoh: Lapor SPT Masa PPh 21 DJP / Bayar Hutang Supplier"
                  class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                />
              </div>

                <div class="mt-4">
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Deskripsi / Catatan Transaksi</label>
                  <textarea v-model="eventDescription" placeholder="Catatan nomor invoice/faktur atau nomor rekening penerima" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" rows="3"></textarea>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Lokasi / Bank / Entitas</label>
                    <input v-model="eventLocation" placeholder="Bank Mandiri / KPP Pajak / Kantor" type="text" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">URL Bukti / Portal DJP</label>
                    <input v-model="eventMeetingUrl" placeholder="https://djponline.pajak.go.id" type="url" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <div class="flex items-center justify-between mb-1.5">
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">Kategori Keuangan</label>
                      <button
                        type="button"
                        @click="isCategoryModalOpen = true"
                  ...
                  <div>
                    ...
                    <select v-model="eventVisibility" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                      <option value="private">Privat (Hanya Akuntan)</option>
                      <option value="public">Publik (Seluruh Manajemen)</option>
                    </select>
                  </div>
                </div>
...
const currentPageTitle = ref('Kalender Keuangan')
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      events.value = [
        {
          id: 'fin-1',
          title: '🧾 Batas Lapor SPT Masa PPh 21/23 (DJP Coretax)',
          start: `${y}-${m}-20T16:00:00`,
          allDay: false,
          extendedProps: {
            calendar: 'danger',
            event_type: 'Pajak PPh / PPN',
            description: 'Batas akhir unggah SPT Masa PPh Masa unifikasi & pelaporan DJP Online',
            location: 'Portal DJP Coretax',
            meeting_url: 'https://djponline.pajak.go.id',
            visibility: 'public'
          }
        },
        {
          id: 'fin-2',
          title: '💸 Jatuh Tempo Hutang Supplier: PT Kimia Sentosa',
          start: `${y}-${m}-15T10:00:00`,
          allDay: false,
          extendedProps: {
            calendar: 'warning',
            event_type: 'Hutang Vendor (AP)',
            description: 'Pembayaran tagihan invoice AP-2026-089 senilai Rp 85.400.000 via Mandiri Virtual Account',
            location: 'Bank Mandiri',
            visibility: 'private'
          }
        },
        {
          id: 'fin-3',
          title: '🏦 Jatuh Tempo Piutang Customer: CV Mega Perkasa',
          start: `${y}-${m}-18T17:00:00`,
          allDay: false,
          extendedProps: {
            calendar: 'primary',
            event_type: 'Jatuh Tempo Piutang (AR)',
            description: 'Penagihan invoice penjualan SO-2026-0312 termin TOP 30 hari',
        ...
        {
          id: 'fin-4',
          title: '📊 Penutupan Buku & Rekonsiliasi Bank Bulanan',
          start: `${y}-${m}-28`,
          end: `${y}-${m}-30`,
          allDay: true,
          extendedProps: {
            calendar: 'purple',
            event_type: 'Tutup Buku & Rekonsiliasi',
            description: 'Rekonsiliasi mutasi bank, jurnal penyesuaian depresiasi aset, dan laporan Laba Rugi bulanan',
            location: 'Ruang Akuntansi',
            visibility: 'public'
          }
        }
      ]
