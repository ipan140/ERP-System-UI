  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Consolidation" />
...
import { consolidationService } from '@/services/finance/consolidation.service'
...
const fetchData = async () => {
  ...
  try {
    const data = await consolidationService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await consolidationService.update(formData.value.id, formData.value)
    } else {
      await consolidationService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
...
const deleteRecord = async (id: number) => {
  ...
  try {
    await consolidationService.delete(id)
    fetchData()
  } catch (err: any) {
    alert(err.response?.data?.message || err.message)
  }
