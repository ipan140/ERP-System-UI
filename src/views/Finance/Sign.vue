...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(API_BASE_URL + '/finance/sign', {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? API_BASE_URL + '/finance/sign/' + formData.value.id : API_BASE_URL + '/finance/sign'
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(API_BASE_URL + '/finance/sign/' + id, {
      method: 'DELETE',
