  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Spreadsheet Bi" />
...
import { spreadsheetBiService } from '@/services/finance/spreadsheet-bi.service'
...
const fetchData = async () => {
  ...
  try {
    const data = await spreadsheetBiService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await spreadsheetBiService.update(formData.value.id, formData.value)
    } else {
      await spreadsheetBiService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
...
const deleteRecord = async (id: number) => {
  ...
  try {
    await spreadsheetBiService.delete(id)
    fetchData()
  } catch (err: any) {
    alert(err.response?.data?.message || err.message)
  }
