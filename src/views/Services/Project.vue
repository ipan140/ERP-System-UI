...
import type { IProjectDto } from '@/types/services'

const records = ref<IProjectDto[]>([])
...
const formData = ref<IProjectDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await projectService.update(formData.value.id, formData.value)
    } else {
