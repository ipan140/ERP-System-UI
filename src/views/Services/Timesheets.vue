...
const formData = ref<ITimesheetDto | any>({ name: '', description: '' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const data = await timesheetsService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await timesheetsService.update(formData.value.id, formData.value)
    } else {
      await timesheetsService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
