  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Helpdesk" />
...
import type { IHelpdeskCannedResponseDto } from '@/types/services'

const records = ref<IHelpdeskCannedResponseDto[]>([])
...
const formData = ref<IHelpdeskCannedResponseDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await helpdeskService.update(formData.value.id, formData.value)
    } else {
      await helpdeskService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
