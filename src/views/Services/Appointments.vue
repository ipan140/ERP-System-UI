...
const records = ref<IAppointmentDto[]>([])
...
const formData = ref<IAppointmentDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await appointmentsService.update(formData.value.id, formData.value)
    } else {
      await appointmentsService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
