    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-xs">
      <div>
        <h2 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <span class="p-1.5 rounded-lg bg-amber-50 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 text-sm">📦</span>
          Kalender Logistik, Gudang & Pengadaan (Supply Chain Calendar)
        </h2>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
          Pelacakan estimasi kedatangan bahan baku (ETA PO), jadwal pengiriman pesanan (DO), stock opname fisik gudang, dan perawatan mesin.
        </p>
      </div>
      ...
      <Modal v-if="isOpen" @close="closeModal">
        <template #body>
          ...
          >
            ...
            >
              {{ selectedEvent ? 'Edit Jadwal Rantai Pasok' : 'Tambah Jadwal Rantai Pasok' }}
            </h5>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              Jadwalkan ETA pengiriman vendor, jadwal dispatch kurir/truk, siklus stock opname, atau jadwal kalibrasi mesin.
            </p>

            <div class="mt-8">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  Judul Aktivitas Supply Chain
                </label>
                <input
                  v-model="eventTitle"
                  type="text"
                  placeholder="Contoh: ETA Kedatangan Bahan Baku PO-2026 / Stock Opname Rak A"
                  class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                />
              </div>

                <div class="mt-4">
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Deskripsi / No. Resi / Plat Truk</label>
                  <textarea v-model="eventDescription" placeholder="Detail muatan, no. kontainer, manifes logistik, atau nama armada pengemudi" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" rows="3"></textarea>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Gudang / Lokasi Dock</label>
                    <input v-model="eventLocation" placeholder="Gudang Utama - Loading Dock B" type="text" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                  <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">URL Tracking Logistik</label>
                    <input v-model="eventMeetingUrl" placeholder="https://tracking.ekspedisi.com/no-resi" type="url" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                  </div>
                </div>
                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <div class="flex items-center justify-between mb-1.5">
                      <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">Kategori Rantai Pasok</label>
                      <button
                        type="button"
                        @click="isCategoryModalOpen = true"
                  ...
                  <div>
                    ...
                    <select v-model="eventVisibility" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                      <option value="private">Privat (Hanya Tim Gudang)</option>
                      <option value="public">Publik (Seluruh Tim Operasional)</option>
                    </select>
                  </div>
                </div>
...
const currentPageTitle = ref('Kalender Rantai Pasok')
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      events.value = [
        {
          id: 'scm-1',
          title: '🚚 ETA Kedatangan Bahan Baku: PT Steelindo (PO-2026-044)',
          start: `${y}-${m}-12T09:30:00`,
          allDay: false,
          extendedProps: {
            calendar: 'warning',
            event_type: 'ETA Supplier (PO)',
            description: 'Pengiriman 20 ton pelat baja tahan karat. Driver: Pak Joko (B 9812 UIK)',
            location: 'Gudang Utama - Loading Dock B',
            visibility: 'public'
          }
        },
        {
          id: 'scm-2',
          title: '📦 Dispatch DO: Pengiriman Armada Wingbox ke Surabaya',
          start: `${y}-${m}-14T08:00:00`,
          allDay: false,
          extendedProps: {
            calendar: 'primary',
            event_type: 'Pengiriman (DO)',
            description: 'Pengiriman 500 karton barang jadi pesanan distributor Jawa Timur',
            location: 'Ekspedisi Sentral Kargo',
            meeting_url: 'https://tracking.sentralkargo.com/DO-4421',
            visibility: 'public'
          }
        },
        {
          id: 'scm-3',
          title: '🔍 Stock Opname Bulanan Gudang Bahan Baku',
          start: `${y}-${m}-25`,
          end: `${y}-${m}-27`,
          allDay: true,
          extendedProps: {
            calendar: 'danger',
            event_type: 'Stock Opname Gudang',
            description: 'Penghitungan fisik menyeluruh (cycle count) rak A1 sampai E12 gudang pusat',
        ...
        {
          id: 'scm-4',
          title: '🛠️ Kalibrasi & Servis Rutin Mesin CNC Milling #02',
          start: `${y}-${m}-28T13:00:00`,
          end: `${y}-${m}-28T17:00:00`,
          allDay: false,
          extendedProps: {
            calendar: 'success',
            event_type: 'Perawatan Mesin',
            description: 'Maintenance preventif berkala oleh teknisi vendor resmi Fanuc',
