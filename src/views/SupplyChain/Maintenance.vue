...
import type { IMaintenanceEquipmentDto } from '@/types/supply-chain'

const records = ref<IMaintenanceEquipmentDto[]>([])
...
const formData = ref<IMaintenanceEquipmentDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await maintenanceService.update(formData.value.id, formData.value)
    } else {
