  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Quality" />
...
import type { IQualityCheckDto } from '@/types/supply-chain'

const records = ref<IQualityCheckDto[]>([])
...
const formData = ref<IQualityCheckDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await qualityService.update(formData.value.id, formData.value)
    } else {
      await qualityService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
