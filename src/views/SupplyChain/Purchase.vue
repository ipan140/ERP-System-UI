...
import type { IProductSupplierInfoDto } from '@/types/supply-chain'

const records = ref<IProductSupplierInfoDto[]>([])
...
const formData = ref<IProductSupplierInfoDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await purchaseService.update(formData.value.id, formData.value)
    } else {
