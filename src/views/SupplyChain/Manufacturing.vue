...
import type { IMrpBomDto } from '@/types/supply-chain'

const records = ref<IMrpBomDto[]>([])
...
const formData = ref<IMrpBomDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await manufacturingService.update(formData.value.id, formData.value)
    } else {
      await manufacturingService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
