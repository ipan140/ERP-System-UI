...
import type { IPlmEcoDto } from '@/types/supply-chain'
...
const formData = ref<IPlmEcoDto | any>({ name: '', description: '' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const data = await plmService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await plmService.update(formData.value.id, formData.value)
    } else {
      await plmService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
