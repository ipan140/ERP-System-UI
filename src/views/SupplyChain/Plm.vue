...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(API_BASE_URL + '/supply_chain/plm', {
...
const saveRecord = async () => {
  ...
  try {
    ...
    const url = isEdit ? API_BASE_URL + '/supply_chain/plm/' + formData.value.id : API_BASE_URL + '/supply_chain/plm'
...
const deleteRecord = async (id: number) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(API_BASE_URL + '/supply_chain/plm/' + id, {
      method: 'DELETE',
