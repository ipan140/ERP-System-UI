  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Barcode" />
...
import type { IBarcodeNomenclatureDto } from '@/types/supply-chain'

const records = ref<IBarcodeNomenclatureDto[]>([])
...
const formData = ref<IBarcodeNomenclatureDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await barcodeService.update(formData.value.id, formData.value)
    } else {
      await barcodeService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
