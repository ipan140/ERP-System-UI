  ...
  <AdminLayout>
    <div class="space-y-6">
      <div class="flex items-start justify-between mb-6">
        <PageBreadcrumb pageTitle="Elearning" />
...
import type { ICertificationDto } from '@/types/website'

const records = ref<ICertificationDto[]>([])
...
const formData = ref<ICertificationDto | any>({ name: '', description: '' })

const fetchData = async () => {
  isLoading.value = true; error.value = null
  try {
    const data = await elearningService.getAll()
    records.value = data
  } catch (err: any) {
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await elearningService.update(formData.value.id, formData.value)
    } else {
      await elearningService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
