...
import type { ICartDto } from '@/types/website'

const records = ref<ICartDto[]>([])
...
const formData = ref<ICartDto | any>({ name: '', description: '' })
...
const saveRecord = async () => {
  ...
  try {
    if (modalMode.value === 'edit' && formData.value.id) {
      await ecommerceService.update(formData.value.id, formData.value)
    } else {
      await ecommerceService.create(formData.value)
    }
    closeModal()
    fetchData()
  } catch (err: any) {
